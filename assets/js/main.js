...
function projectPage() {
  ...
  root.innerHTML = `
    ...
    ${studio ? `<section class="case-credit wrap rv-el">
...
/* Every Vimeo frame is a whole web page and a phone cannot hold many.
   The card nearest the middle of the screen gets the player; the one
   before it is unloaded outright, not merely paused. */
function soloEmbeds() {
  if (!TOUCH) return;
  let current = null;
  ...
  const queue = () => {
    if (waiting) return;
    waiting = true;
    requestAnimationFrame(() => { waiting = false; pick(); });
  };
...
async function warmEmbed(frame) {
  ...
  try {
    if (url.hostname === 'player.vimeo.com') {
      if (!frame.getAttribute('src')) frame.src = url.href;   /* seed yenidən yüklənmir */
